...
import { type Asset, type Kind, type Stage, stages, labels, categoryLabel, filterAssets, sortAssets, pageSlice } from '@/lib/inventory';
...
const initialFilters = { kind: 'texture' as Kind, query: '', category: 'all', stage: 'all' };
...
export default function Home() {
  ...
  const rows = useMemo(() => assets ? filterAssets(assets, filters) : [], [assets, filters]);
  ...
  return <div className="app-shell">
    ...
    <main id="overview" className="main-content">
      ...
      <div className="work-notice"><span className="next-label">IN FOCUS</span><div><strong>One coherent Meadows scene.</strong><span> Roof atlas tested. Straw fringe, complete buildings, and weather are next.</span></div><a href="#milestones" aria-label="View current milestones"><ArrowUpRight size={20}/></a></div>
      ...
      <div className="lower-grid">
        ...
        <section className="panel validation-panel" id="validation"><div className="section-heading"><div><p className="eyebrow">QUALITY & PERFORMANCE</p><h2>Validation ledger</h2></div><ShieldCheck size={22} className="gold"/></div>
          <div className="validation-body">
            ...
            <div className="staging-note"><strong>In the workshop</strong><p>Cutout adapter: {project.staged_native_binding_checks} native checks passed in staging. Straw fringe: authored at 512px; UV, compressed mips and game tests pending.</p><a className="text-link" href={evidenceUrl(project.staged_evidence)} target="_blank" rel="noreferrer">Read staged test summary <ArrowUpRight size={14}/></a></div><p className="small-note">Selective high-resolution textures and crisp presentation on 4K displays remain art goals. Whole-world coverage, weather, LOD transitions, and model or shader streaming still need validation.</p>
    ...
    <Sheet open={!!selected} onOpenChange={open=>{if(!open)setSelected(null);}}>
      <SheetContent className="w-full sm:max-w-xl overflow-y-auto p-2">
        ...
        {selected && <div className="asset-detail">
          ...
          <p className="small-note">These are asset names and metadata. No original texture pixels or mesh geometry are hosted.</p>
        </div>}
      </SheetContent>
    </Sheet>
  </div>;
